...
def getTimeFeatures(sample):
    ...
    从采样信号中提取时域特征。
    @param sample：数组，大小为（采样点数X59）的样本信号
    返回：阵列，大小为（59X10）的时域特征矩阵
    """
    # mean absolute value 平均绝对值，axis=0 按照列计算
    # [..., np.newaxis] 行变列
    mav = abs(sample).mean(axis=0)[..., np.newaxis]
    # # variance 方差
    # varv = np.var(sample, axis=0)[..., np.newaxis]
    # # # 最小值
    # minv = np.min(sample, axis=0)[..., np.newaxis]
    # # # root mean square 均方根
    # rms = np.sqrt(np.mean(sample**2, axis=0))[..., np.newaxis]
    # # # 最大值
    # maxv = np.max(sample, axis=0)[..., np.newaxis]
    # # 标准差
    ...
    # # 峰值
    # Peak = np.max([np.abs(maxv), np.abs(minv)], axis=0)
    # # skewness 偏斜
...
def getNetFeatures(matrix):
    ...
    # Step 2: 计算各个网络特征
    # 节点度 (Degree)
    # degree = np.array(list(dict(G.degree()).values()))[..., np.newaxis]  # (n_nodes, 1)
