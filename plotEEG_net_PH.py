...
# 设置工作目录
os.chdir('E:/lin/stroke/stroke_GCN')
...
# 自定义纵轴标签，并设置字体大小和加粗，指定字体为 'Times New Roman'
# ax1.set_yticklabels([0, 10, 20, 30], fontsize=font_size_ticks, fontweight='bold', fontname='Times New Roman')
# ax1.set_xticks([0, 10, 20, 30])
# ax1.set_xticklabels([0, 10, 20, 30], fontsize=font_size_ticks, fontweight='bold', fontname='Times New Roman')
# 移除横纵坐标的刻度和小短线
...
# 绘制三维点云，增加点的大小和调整颜色
ax.scatter(points[:, 0], points[:, 1], points[:, 2], c='b', marker='o', s=60, alpha=0.7)

# 设置坐标轴标签
ax.set_xlabel('X', fontsize=18, labelpad=15)  # labelpad 增加与轴的距离
...
# 设置标题
ax.set_title('3D Point Cloud', fontsize=20, pad=20)

# 手动调整边框清晰度
ax.grid(False)  # 关闭默认网格
ax.xaxis.line.set_linewidth(2)  # 设置x轴线条宽度
ax.yaxis.line.set_linewidth(2)  # 设置y轴线条宽度
ax.zaxis.line.set_linewidth(2)  # 设置z轴线条宽度
y_ticks = [-2.0, -1.0, 0.0, 1.0, 2.0]  # 设置y轴刻度位置（根据数据范围调整）
ax.set_yticks(y_ticks)  # 应用到y轴
z_ticks = [-1.5, -0.5, 0.5, 1.5]  # 设置y轴刻度位置（根据数据范围调整）
ax.set_zticks(z_ticks)  # 应用到y轴
# 显示图形
plt.tight_layout()
# plt.savefig("eeg_points_cloud.png", dpi=300)
plt.show()
#%%
# score
import numpy as np
...
import pandas as pd
import matplotlib.pyplot as plt
os.chdir(r"E:\lin\stroke\stroke_GCN")
# 设置全局字体样式
# plt.rcParams['font.family'] = 'DejaVu Sans'
# plt.rcParams['font.serif'] = ['Times New Roman']
