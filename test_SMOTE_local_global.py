...
if __name__ == "__main__":
    ...
    feature = np.load(resultpath + f'{netc}_localfeature.npy')
    y = np.load(resultpath + 'y_label.npy', allow_pickle=True)
    file_path = resultpath + netc + '_network.npy'

    # 读取测试的患者ID
    f = open(resultpath + f'test_indices_{netc}_local.txt', 'r')
    ...
    model = EEGGCN()
    state_dict = torch.load(resultpath + "model" + "/" + netc + "_kf_9_local" + '.pth')
