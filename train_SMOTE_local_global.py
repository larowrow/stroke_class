...
def test(fold, epoch, testloader=None, training=True):
    '''
    全局变量，顾名思义，就是全局都可以使用的变量。一般定义在函数之外，然后函数内部通过global关键字描述，以区分函数内部的普通变量。
    首先在函数外部进行变量初始化，然后在函数内部通过global关键字呼叫这个变量，就可以实现全局变量的功能了。看输出就知道了，
    变量会随着函数的不断运行也在不断变化。
    '''
    ...
    bestModel = os.path.join(resultpath, "model", f"{connetc}_kf_{fold}_global.pth")
...
if __name__ == "__main__":
    ...
    # 31*15的特征
    feature = np.load(resultpath +f'{connetc}_globalfeature.npy')
    ...
    # 输出测试样本的索引
    with open(resultpath + f'test_indices_{connetc}_global.txt', 'w') as file_handler:
